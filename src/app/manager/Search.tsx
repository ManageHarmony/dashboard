...
const SearchBar: React.FC<SearchBarProps> = ({ onSearch }) => {
    ...
    const fetchSuggestions = useCallback(
        debounce(async (searchQuery: string) => {
            ...
            try {
                ...
                const response = await fetch(`https://harmony-backend-z69j.onrender.com/api/manager/${managerId}/search/bar?query=${encodedQuery}`);
                if (response.ok) {
                    ...
                    const allSuggestions = [
                        ...(data?.managerDetails?.creators || []).map((item: any) => ({
                            name: item.username || item.email,
                            id: item.id,
                            role: 'creator',
                        })),
                        ...(data?.managerDetails?.doctors || []).map((item: any) => ({
                            name: item.username,
                            id: item.id,
                            role: 'doctor',
                        })),
    ...
    const handleSuggestionClick = (suggestion: { name: string, id: string, role: string }) => {
        ...
        switch (suggestion.role.toLowerCase()) {
            case 'creator':
                router.push(`/manager/staff/cards/${suggestion.id}?role=creator`);
                break;
            case 'doctor':
                router.push(`/manager/staff/cards/${suggestion.id}?role=doctor`);
                break;
            default:
